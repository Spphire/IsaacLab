...
class HandArmManipulationEnv(DirectRLEnv):
    ...
    def _reset_target_pose(self, env_ids):
        ...
        rand_floats = sample_uniform(-1.0, 1.0, (len(env_ids), 3), device=self.device)*0.15
        self.goal_pos[env_ids] = self.goal_pos_init[env_ids] + rand_floats
...
def compute_rewards(
    ...
    # Total reward
    if subepoch<subepoch_threshold:
        temp = forearm_linvel_error*forearm_linvel_error_weight+\
                forearm_rotvel_error*forearm_rotvel_error_weight+\
                    forearm_rot_error*forearm_rot_error_weight+\
                        pos_error*pos_error_weight
    else:
        temp = keypoints_error
    ...
    if subepoch<subepoch_threshold:
        success_mask = (pos_error <= pos_success_threshold) & \
                   (forearm_rotvel_error < forearm_rotvel_success_threshold)
    else:
